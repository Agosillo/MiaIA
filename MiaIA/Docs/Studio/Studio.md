...
An English/Italian online command assistant is included in normal Studio builds and remains disabled at runtime until the user chooses a language and selects the explicitly labelled Wit.ai/Meta checkbox. Its controls sit above the Console output: the first row contains online and exact-confidence choices, while the second contains language, status, and Development-only settings. Development settings accept one Client Access Token per language; a saved per-user override takes effect without recompilation, and replacing a token selects another private Wit.ai app. Shipping and Store builds hide those credential controls and use release-packaged defaults. Typed natural language is sent to the selected language's application, converted locally into a validated existing Console command, and displayed with intent and confidence; execution requires `Confirm command` or one exact local affirmative response. Negative responses discard it, and neither type of response is sent to Wit.ai. A separate non-persistent Confident mode can auto-execute only proposals whose intent and every entity have exact `100%` provider confidence; it starts disabled because statistical confidence is not proof. The provider never bypasses `MiaIACommandProcessor`; its language-specific tokens and transient assistant state are Studio runtime concerns rather than project data. The current slice is text-only; microphone capture is a later provider input. The [online assistant guide](OnlineAssistant.md) documents Visual Studio builds, client-token configuration, packaging, Wit.ai intents, and the public-release security boundary.
