...
An English/Italian command assistant is included in normal Studio builds and remains disabled until explicitly enabled. Its default **MiaIA Local** provider runs offline in portable C++ and needs no token; `Auto` accepts English, Italian, and mixed phrases in the same session. Confirmed non-exact proposals become validated local examples, while unknown or explicitly incorrect interpretations wait in a supervised review queue and never affect recognition until classified. The Console sidebar switches between exact commands, language-aware assistant examples, and the Local learning manager; the latter lists pending and validated phrases and supports reassignment, deletion, backup, restore, and reset. The provider selector retains **Wit.ai (experimental)** as an optional online comparison; its Development-only settings accept one Client Access Token per language, while Shipping hides credential controls and uses release-packaged defaults. Typed natural language becomes a validated existing Console command and is displayed with intent and confidence; execution requires `Confirm command` or one exact local affirmative response. A separate non-persistent Confident mode can auto-execute only proposals whose intent and every entity have exact `100%` provider confidence. No provider bypasses `MiaIACommandProcessor`, and the versioned local corpus remains outside `.mai` project data. The current slice is text-only; microphone transcription is a later provider input. The [command assistant guide](OnlineAssistant.md) documents the local architecture, bilingual tests, Visual Studio builds, optional Wit.ai configuration, and the shared safety boundary.
