...
- one complete refresh boundary over `MiaIAClient` plus a lightweight context-state refresh used to detect external active-context changes;
- lightweight project-context summaries plus create, fork, select, rename, and remove operations;
- automatic clearing of context-specific selection, inspection, trace, diagnostic, checkpoint-selection, and timeline presentation state when the active context changes;
...
The `Training timeline` tab presents the current controlled-session status, epoch and sample cursor, completed and total steps, sequential/shuffled order, deterministic seed when applicable, learning rate, loss, optimizer, active phase-debug boundary, and latest breakpoint stop. The cursor distinguishes the actual next dataset sample from its position in the current epoch permutation. Its six-phase strip follows Before, Forward, Backward, Update, Verify, and Commit; completed phases use the debug color and a yellow upward cursor identifies the exact current boundary. The committed-step history is newest first and uses the Engine-owned history summaries, so every row reports stable step, epoch and sample identities, loss before and after, and update counts without duplicating training calculations. The frontend bounds its widget list to the newest 200 entries while leaving the native retained history unchanged. Selecting a visible row requests only that retained `TrainingStepSnapshot` and displays its targets, predictions, errors, loss delta, learning rate, optimizer, weight updates, and bias updates. `Clear view` hides the currently displayed rows and selected detail without deleting native history; later committed steps appear normally, and a replaced session resets the view filter. The selected step survives ordinary refreshes while it remains in the current session and is cleared automatically when the session history is replaced.
...
An English/Italian command assistant is included in normal Studio builds and remains disabled until explicitly enabled. Its default **MiaIA Local** provider runs offline in portable C++ and needs no token; `Auto` accepts all installed languages and mixed phrases in the same session. Portable Language Packs can add a third, fourth, or later language by exporting an editable translation template from **Review** and installing it after translation, without recompiling Studio. Confirmed non-exact proposals become validated local examples, while unknown or explicitly incorrect interpretations wait in a supervised review queue and never affect recognition until classified. The Console sidebar switches between exact commands, language-aware assistant examples, and the Local learning manager; the latter lists pending and validated phrases and supports reassignment, deletion, backup, restore, reset, template export, and pack installation. The provider selector retains **Wit.ai (experimental)** as an optional online comparison; its Development-only settings accept one Client Access Token per language, while Shipping hides credential controls and uses release-packaged defaults. Typed natural language becomes a validated Console command or allowlisted Studio view action and is displayed with intent and confidence; execution requires `Confirm command` or one exact local affirmative response. A separate non-persistent Confident mode can auto-execute only proposals whose intent and every entity have exact `100%` provider confidence. Model/data proposals use `MiaIACommandProcessor`; the finite Studio view-action catalog uses existing view/layout handlers after the same proposal validation and confirmation, and the versioned local corpus and Language Packs remain outside `.mai` project data. The current slice is text-only; microphone transcription is a later provider input. The [command assistant guide](OnlineAssistant.md) documents the local architecture, Language Pack workflow, tests, Visual Studio builds, optional Wit.ai configuration, and the shared safety boundary.

In the Unreal host, the panel addresses this state through an opaque `FMiaIAInstanceHandle`. `FMiaIAInstanceService` owns the corresponding controller in the runtime `IDE` module and performs every stateful call there, including model-context management, forward- and backward-trace playback, and training-timeline selection, so Blueprint, the integrated CLI, and `IDEStudio` observe one SDK project state even though Unreal builds them across multiple DLL modules. Only the `default` frontend instance exists today, but its native `ProjectState` can own several isolated contexts. The shared editor/standalone toolbar lists them and can create, fork, select, rename, or remove the active context. Independently executing SDK contexts remain future work.
...
`Clear all` update only the active context's store. `.mai` version 3 persists each context's
...
The shared toolbar uses two stable functional rows rather than overflowing one horizontal strip. The first row begins with a `Project` menu for New, Open, Save, Save as, Import ONNX, Export ONNX, and Info, followed by the active-context selector. The selector lists topology and checkpoint counts, creates and immediately selects a named empty context, or uses `Create experiment from active model` to fork the active non-empty context under the entered name. A fork selects an independent context with the same network identities and experiment setup but clean execution and checkpoint state; it also preselects the source as the comparison Reference. The selector switches contexts, renames the active context, and removes it only after confirmation while retaining at least one. A switch clears frontend state that belongs to the previous context before rebuilding the topology and lower tools. `.mai` operations preserve every context, while ONNX operations exchange only the active context's supported model portion. The `Layout` menu shares persistent Expanded/Packed placement, Horizontal/Vertical flow, uniform neuron scale, spacing, connection visibility, and All/Selected connection display between 2D and 3D. `Theme` controls interface surfaces and text, while `Colors` independently selects the shared semantic visualization palette. MiaIA Classic preserves the original theme-aware mapping; High Contrast, Color-blind Safe, Monochrome, and ten persisted custom colors apply immediately to both renderers, legends, traces, selection, debug emphasis, signal-health diagnostics, and timeline state. Performance preferences, help, and host actions follow on the same row. Training and phase-debug actions share the second row with their live session and phase status. Path-based operations use a themed in-panel prompt so the editor and packaged application retain identical behavior without an editor-only file-dialog dependency.
