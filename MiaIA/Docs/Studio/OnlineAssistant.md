# MiaIA Studio online command assistant

## Scope

The first command-assistant slice belongs to MiaIA Studio, not to the mathematical
Engine or public SDK. It accepts natural English text, asks Wit.ai to identify one
intent and its entities, converts that structured result into an existing MiaIA
Console command, and displays the exact proposal for confirmation.
...
removes the proposal. Confirmation responses are handled locally and are not sent to
Wit.ai. The normal Console remains available when the assistant is disabled.
...
No network, dataset, checkpoint, or project content is sent by this first slice. The
request contains only the text entered while the clearly labelled online-assistant
checkbox is enabled. Assistant state and credentials are not stored in `.mai` files.

## Build and Visual Studio

The integration is part of a normal MiaIA Studio build. `IDEStudio.Build.cs` defines
`MIAIA_WITH_WIT_AI=1` by default and links Unreal's HTTP and JSON modules, so Visual
Studio does not need to inherit an enablement variable. The provider-neutral command
translation lives in the native `StudioCore` library, while the HTTP provider and UI
live in Unreal. After changing either assistant layer, first open `MiaIA.sln` and build
...
An intentionally offline build can still exclude the provider by setting
`MIAIA_WITH_WIT_AI=0` before Unreal Build Tool runs. The normal Windows packaging
script includes it unless `-DisableWitAI` is passed:
...
change how Console input is interpreted. In Development builds, select **Settings**,
...
The translation layer is provider-neutral and lives in `IDE/StudioCore`; only the
current HTTP adapter is private Unreal code. A future microphone path can feed Wit.ai
speech understanding into the same proposal contract. Its transcription can also use
the same local confirmation vocabulary before execution. A future local MiaIA or
speech provider can implement the same interface without changing command execution
or the Engine.
