...
The reader retains explicit version-1 compatibility and migrates an accepted v1 archive to context `1` named `Model 1`; the next ordinary save publishes v2. Opening still constructs and validates complete replacement state before publishing it, and saving still uses sibling-file replacement. Multi-context round-trip tests cover isolated networks, datasets, session configuration, breakpoints, checkpoints, active selection, empty contexts, monotonic identifiers, and v1 migration.
