...
The Unreal frontend then separated the active renderer from its model visualization choice. Classic remains the default for both 2D and 3D, while Coaxial Rings distributes each expanded layer around a parallel ring on one common horizontal axis. Ring radii derive from neuron count, uniform sphere size, and configured gap so automatic placement remains non-overlapping.

The second additional 3D choice, Spiral Tokens, represents each layer as one thin opaque token. Terminal points fill both faces on concentric rings with no central point; ring capacities grow as 8, 16, 24, and so on, with alternating half-step offsets. Corresponding input and output terminals share one neuron ID, and the token body replaces any artificial internal connection. Token radii follow the outer occupied ring instead of retaining a large fixed minimum, while the reduced thickness keeps layers visually light. Real weights are rendered only from a source token's output face to the destination token's input face. Flat low-tessellation token and terminal cylinders remain in the existing aggregated dynamic mesh, making the view lighter than duplicating neuron spheres. The persisted per-view selection reuses existing IDs, hit testing, Inspector, traces, semantic colors, fit, and reset behavior without changing StudioCore, Engine, SDK, CLI, or `.mai` data. A separate token-body hit target now selects and moves the complete layer as one visual group; a marquee covering one complete layer promotes its neuron set to the same persistent layer selection. The complete token uses the unconditional semantic selection color at every zoom and token size. Once selected, a threshold distinguishes a terminal click from a drag begun anywhere on the token, so single-neuron inspection remains available while both faces, labels, and connection endpoints move only as one layer. Fit includes moved token bounds and Reset layout clears the Unreal-only layer offsets.

Compact layer summaries in both Unreal renderers now reuse the whole-network preview vocabulary. Each aggregate layer displays blue input indicators derived from its predecessor width and green output indicators derived from its own neuron count; the first layer uses its own width on both sides. Horizontal, Vertical, Forward, and Reverse layouts orient the bundles with model flow, while automatic spacing and fit include their capped visible extents.
...
### Multi-context project state

The SDK's former single network, dataset, training/debug sessions, and checkpoint store were grouped into an explicit `ModelContext`. A process-local `ProjectState` now owns multiple contexts under stable runtime IDs and maintains one active selection. Existing network operations target the active context, while new SDK plus shared CLI operations create, list, select, rename, and remove isolated contexts. The term replaced the earlier `ModelInstance`: the neural model is materialized only when a network is created or imported, whereas its context may exist and be saved while empty.

Context selection is protected by the same synchronization and mutation boundary as training: a Running session or active phase-debug transaction must be paused or cancelled before creating, selecting, or removing a context. The single cooperative worker still prevents parallel training. At this stage `.mai` v1 remained a one-network format, so saving multiple runtime contexts was rejected rather than losing inactive state.

### `.mai` v2 and graphical model-context selection

The project archive advanced to version 2 as one ordered tagged stream. A project-level section records the active and next context IDs, and each context section records its stable ID, name, optional current ONNX network, dataset reference, training configuration, breakpoints, and checkpoint count. Each checkpoint then stores its stable ID, name, and independent supported ONNX payload. Empty contexts remain valid, next context and checkpoint identifiers survive a round trip, and large ONNX payloads stream through temporary files rather than being duplicated in metadata buffers.

The reader retains explicit version-1 compatibility and migrates an accepted v1 archive to context `1` named `Model 1`; at that stage the next ordinary save published v2. Opening still constructs and validates complete replacement state before publishing it, and saving still uses sibling-file replacement. Multi-context round-trip tests cover isolated networks, datasets, session configuration, breakpoints, checkpoints, active selection, empty contexts, monotonic identifiers, and v1 migration.

StudioCore gained the same context lifecycle operations and lightweight context state as the SDK. The shared Unreal editor/standalone toolbar now lists project contexts and can create, select, rename, or remove them with the normal mutation guard and a removal confirmation. A context switch clears stale selection, Inspector, trace, diagnostic, checkpoint-selection, and timeline presentation state before rebuilding the active topology. Unreal Blueprint gained reflected `ModelContext` snapshots and lifecycle nodes, while project information now exposes context count, active identity, and active checkpoint count.

### Immutable model-context comparison

Multi-context projects gained a read-only comparison path independent of checkpoint ownership. A renderer-neutral Engine comparator validates both networks, summarizes their topology, and requires matching stable layer, neuron, and connection identities before producing activation-type, bias, and weight differences. The SDK resolves both contexts without selecting them and preserves the caller's result on missing, identical, empty, or invalid inputs. The shared CLI exposes this as `model compare <reference-id> <current-id> [limit]` with explicit topology output and bounded rankings.

StudioCore retains a successful result only as transient presentation state. The shared Unreal panel adds Current and Reference selectors, a configurable ranking limit, clear empty/incompatible explanations, and a removable overlay. Detailed 2D and 3D views use the same signed `Current - Reference` bias and weight maps, while unchanged elements are subdued. The selected pair and derived result remain outside the `.mai` project format.

### Model experiment forks

Multi-context management gained an atomic fork operation across `ProjectState`, SDK, shared CLI, StudioCore, Unreal Blueprint, and the shared graphical selector. `model fork <source-id> <name>` creates and selects a new context whose populated network retains the source topology, activation types, stable element IDs, weights, and biases. Dataset values, training configuration, and breakpoint definitions are copied by value, while runtime activations, training progress and history, debug state, breakpoint hits, and checkpoints are reset. The source remains untouched and later mutations are isolated.

Because source and fork retain matching stable network identities, their initial immutable comparison is exact and later parameter divergence maps directly onto the existing 2D/3D overlay. A successful fork is serialized as another ordinary context without requiring a fork-specific section. Tests cover rejected inputs, clean copied state, source isolation, identical and divergent comparisons, Console and StudioCore access, mutation guards, and `.mai` round trips.

### Deterministic shuffled training

Controlled sessions now support deterministic per-epoch sample shuffling with an explicit unsigned 64-bit seed. A project-owned Fisher-Yates implementation uses a fixed SplitMix64 mapping, snapshots expose both the actual sample index and its permutation position, and retained history records the actual execution order. The SDK, shared Console, Blueprint layer, and Studio timeline expose the configuration. `.mai` version 3 persists order mode and seed for every model context, while version 1 and 2 archives remain readable as sequential seed-0 configurations.

### Opt-in online command assistant

MiaIA Studio gained its first provider-neutral natural-language command proposal path. An optional Unreal-private Wit.ai adapter interprets English or Italian text through separate private language applications and runtime tokens, while StudioCore validates the returned intent and entities and converts them into an existing shared Console command. The panel displays the recognized intent, confidence, and exact command and normally requires explicit confirmation before dispatching it through `MiaIACommandProcessor`. A separate default-off Confident mode auto-confirms only when the intent and every returned entity report exact `100%` confidence.

The text assistant is included in normal Studio builds but remains separately disabled at runtime and sends no project or model state. Its language, online, confidence, status, and proposal controls live with the Console input they affect. Development-only settings accept replaceable per-user English and Italian Client Access Tokens; changing a token switches the corresponding private Wit.ai app without recompilation. Development may also supply tokens through the process environment, while release packaging generates an ignored deployment-only default configuration that is retained by the Store MSIX workflow. Shipping hides and ignores the developer credential editor, so customers see no token fields and app rotation occurs through a new package. Credentials and assistant presentation state remain outside `.mai`. Server Access Tokens are explicitly rejected by policy, and a controlled proxy remains the stronger future boundary for quota protection and centralized revocation.

### MiaIA Local command understanding
...
MiaIA Local then gained an automatic bilingual mode and supervised user corpus. Confirmed non-exact proposals become exact validated examples without storing executable command text; unknown and explicitly incorrect phrases remain inert in a review queue until the user assigns an allowlisted intent. The Console sidebar now follows the active mode by showing exact commands, bilingual natural-language examples, or a complete Local learning manager for pending and validated phrases. Associations can be corrected or deleted, and the corpus can be backed up, restored, or reset with confirmation. The portable core owns versioned corpus import/export while the Unreal host stores the data outside `.mai` under its Saved directory. Native coverage verifies mixed-language recognition, confirmation learning, duplicate suppression, safe malformed-data rejection, manual classification, reassignment, removal, reset, incorrect-proposal suppression, and language filtering.
