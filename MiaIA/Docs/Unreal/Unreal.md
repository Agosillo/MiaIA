...
In preview and compact modes, 2D zoom and pan and 3D camera navigation remain available. `Fit view` and `Reset layout` frame the active level. The whole-network glyph uses the same semantics in 2D and 3D; opening it reveals the normal detailed topology when the network is within the configured limits, or the aggregate layer graph when those limits are exceeded. The compact 3D layer axis expands with its layer count and the fitted camera frames that complete extent, preserving a minimum gap between uniformly sized aggregate spheres. Both renderers keep layer aggregates in the active-neuron color and identify the selected layer with the existing semantic selection outline; aggregate markers do not blink. Individual layer nodes are summaries rather than real neurons and are not draggable. Opening one uses the existing atomic SDK layer query rather than bypassing the compact limits with a complete network snapshot; neuron selection and the paged Relationship Explorer then work normally inside the focused layer. Following a cross-layer endpoint opens only its destination layer. Aggregate-layout editing and rendering cross-layer relationship pages directly on the compact scene remain future work.
...
- backward gradient-flow animation;
