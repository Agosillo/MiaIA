...
- **Execution trace** accepts a whitespace-separated input vector and runs the immutable SDK forward trace. The tab displays captured inputs and outputs plus a paged list of the selected neuron's incoming `source activation x weight` contributions. The Inspector adds weighted input, bias, pre-activation, and activation, while 2D and 3D use the same recorded activation and contribution values for their overlay. Clearing the trace restores ordinary model/debug coloring and does not change the network.
